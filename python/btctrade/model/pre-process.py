...
def pre_process_logs(start="2015-11-17/07", end=None):
    ...
    items = union_src.map(lambda x: x.split("\t")).map(lambda x: (long(x[0]), (long(x[0]), float(x[3]), float(x[4]), float(x[5])))).filter(lambda x: x[0] >= 1447745420).reduceByKey(lambda x, y : x)
